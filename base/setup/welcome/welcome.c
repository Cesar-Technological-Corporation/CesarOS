 ...
 *  Copyright (C) 2001, 2002, 2003 ReactOS Team / (C) 2024 Cesar Technological Corporation
 *
 ...
 * PROJECT:     CesarOS "Welcome"/AutoRun application
...
/*
 * Expands the path for the CesarOS Installer "reactos.exe".
    ...
    IN SIZE_T PathSize)
{
    ...
    *lpInstallerPath = 0;
     ...
     * CesarOS installation directory, or from our current directory.
     */
    *lpInstallerPath = 0;
    ...
    IN LPTSTR szAction  OPTIONAL)
{
    ...
    if (szCommand && *szCommand)
    {
        ...
        /* Check for special applications: CesarOS Installer */
    ...
    if (*pTopic->szCommand)
    {
        ...
        {
            /* Check for special applications: CesarOS Shell */
...
LoadLocalizedResourcesFromINI(LCID Locale, LPTSTR lpResPath)
{
    ...
    GetPrivateProfileString(TEXT("Defaults"), TEXT("AppTitle"), TEXT("CesarOS - Welcome") /* default */,
...
LoadConfiguration(VOID)
{
    ...
    if (!LoadString(hInstance, IDS_APPTITLE, szAppTitle, ARRAYSIZE(szAppTitle)))
        StringCchCopy(szAppTitle, ARRAYSIZE(szAppTitle), TEXT("CesarOS - Welcome"));
...
RunAction(INT nTopic)
{
    ...
    if (!pTopics[nTopic]->bIsCommand)
    {
        ...
        {
            MessageBox(hWndMain, Command + 5, TEXT("CesarOS"), MB_OK | MB_TASKMODAL);
            return TRUE;
        }
    }
    else
...
OnPaint(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    StringCchCopy(szVersion, ARRAYSIZE(szVersion),
                  TEXT("CesarOS ") TEXT(KERNEL_VERSION_STR));
