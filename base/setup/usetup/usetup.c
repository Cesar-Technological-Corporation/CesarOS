/*
 *  CesarOS kernel
 *  Copyright (C) 2002, 2003, 2004 ReactOS Team / (C) 2024 Cesar Technological Corporation
 *
 ...
 * PROJECT:         CesarOS text-mode setup
...
// FIXME?? Aren't we going to enter an infinite loop, if this test fails??
    ...
    while (TRUE)
    {
        ...
        else if (Ir->Event.KeyEvent.wVirtualKeyCode == VK_RETURN)  /* ENTER */
        {
            ...
            /*
             * Check whether the user wants to install CesarOS on a disk that
            ...
            {
                PopupError("The disk you have selected for installing CesarOS\n"
...
SelectFileSystemPage(PINPUT_RECORD Ir)
{
    ...
    if (FormatState == Start)
    {
        ...
        if (!SystemPartition)
        {
            ...
            //
            PopupError("The CesarOS Setup could not find a supported system partition\n"
                       "on your system or could not create a new one. Without such partition\n"
                       "the Setup program cannot install CesarOS.\n"
...
InstallDirectoryPage(PINPUT_RECORD Ir)
{
    ...
    if ((RepairUpdateFlag || IsUnattendedSetup) && IsValidPath(InstallDir))
    {
        ...
        /*
         * Check whether the user attempts to install CesarOS within the
...
RunUSetup(VOID)
{
    ...
    while (Page != REBOOT_PAGE && Page != RECOVERY_PAGE)
    {
        ...
        // CONSOLE_SetUnderlinedTextXY(4, 3, " CesarOS " KERNEL_VERSION_STR " Setup ");
